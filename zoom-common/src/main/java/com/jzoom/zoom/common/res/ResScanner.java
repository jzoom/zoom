...
public class ResScanner implements Destroyable {
	...
	public class ClassRes extends FileRes {
		...
		public Field[] getFields() {
			if(fields==null) {
				List<Field> list = Classes.getFields(getType());
		...
		public Method[] getPubMethods() {
			if(methods == null) {
				List<Method> list = Classes.getPublicMethods(getType());
	 ...
	 * @return
	 */
	public Res fetchClass(String className) {
	 ...
	 * @return
	 */
	public Res fetchFile(String name) {
	...
	private static String getClassName(String root, String path) {
		String result = path.substring(root.length(), path.length() - 6).replace("/", ".");
