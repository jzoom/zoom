...
public class MysqlDbStrict implements DbStructFactory {
	...
	public MysqlDbStrict( String dbName,boolean lazyLoad) {
		this.dbName  = dbName;
		this.lazyLoad = lazyLoad;
		tables = new ConcurrentHashMap<String, TableMeta>();
	}
	...
	@Override
	public TableMeta getTableMeta(Ar ar, String table) {
		if(lazyLoad) {
			return createTableMeta(table, ar);
		}else {
			final Map<String, TableMeta> tables = this.tables;
			TableMeta result = tables.get(table);
			if(result == null) {
				synchronized (tables) {
					result = createTableMeta(table,ar);
	...
	private TableMeta createTableMeta(final String table, Ar ar) {
	...
	private void fill( TableMeta meta, Ar ar) {
		List<Record> list = ar.executeQuery("select table_comment as comment,table_name as name from information_schema.tables where table_schema=?", dbName);
		if(list.size() > 0 ) {
			Record record = list.get(0);
			meta.setComment(record.getString("comment"));
		}
		
		list = ar.executeQuery("SELECT TABLE_NAME,COLUMN_NAME,IS_NULLABLE,DATA_TYPE,CHARACTER_MAXIMUM_LENGTH,COLUMN_KEY,EXTRA,COLUMN_COMMENT,COLUMN_DEFAULT FROM information_schema.columns WHERE table_schema=? and TABLE_NAME=?", dbName,meta.getName());
